...
This repository is licensed under the Databricks license in [`LICENSE`](LICENSE) (use is tied to the
Databricks Platform Services). One subtree carries its own license:

- **`adjuster-console/`** is a derivative work of Databricks
  [app-templates](https://github.com/databricks/app-templates) (the `agent-openai-agents-sdk-multiagent`
  template) and is governed by the Databricks License in
  [`adjuster-console/LICENSE`](adjuster-console/LICENSE), with modifications recorded in
  [`adjuster-console/NOTICE`](adjuster-console/NOTICE). For files under `adjuster-console/`, that
  subtree license controls.
